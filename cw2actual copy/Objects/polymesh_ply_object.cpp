...
#include "polymesh_ply_object.h"

using namespace std;

PolyMeshPly::PolyMeshPly(char* file, bool smooth)
{
//BEGIN_STAGE_ONE
    int count;
...
//BEGIN_STAGE_ONE
// Moller-Trumbore
bool PolyMeshPly::rayTriangleIntersect(const Ray& ray, Vector v0, Vector v1, Vector v2, float& t, float& u, float& v)
{
    Vector p;
    Vector d;
    Vector e1, e2, h, s, q;
...
void PolyMeshPly::compute_vertex_normals(void)
{
...
void PolyMeshPly::compute_face_normal(int which_triangle, Vector& normal)
{
    Vector v0v1, v0v2;
...
Hit* PolyMeshPly::triangle_intersection(Ray ray, int which_triangle)
{
...
Hit* PolyMeshPly::intersection(Ray ray)
{
    Hit* hits = 0;
//BEGIN_STAGE_ONE
    int i;
...
void PolyMeshPly::apply_transform(Transform& trans)
{
//BEGIN_STAGE_ONE
    for (int i = 0; i < vertex_count; i += 1)
    {
